    ...
    private class NFCTagReaderSessionDelegateHelper: NSObject, NFCTagReaderSessionDelegate {
      ...
      private func handleConnectedTag(
        ...
        case .miFare(let miFareTag):
          ...
          // For MIFARE Ultralight, try to read data
          if miFareTag.mifareFamily == .ultralight {
            resultText += "MIFARE Ultralight Tag\n"
            // Read the first block (just as an example)
            miFareTag.sendMiFareCommand(commandPacket: Data([0x30, 0x00])) {
              [weak self] data, error in
              if let error = error {
                resultText += "Error reading tag: \(error.localizedDescription)\n"
        ...
        case .iso15693(let iso15693Tag):
          ...
          iso15693Tag.readSingleBlock(requestFlags: .highDataRate, blockNumber: 0) {
      ...
      private func finish(
        ...
        session.alertMessage = "Tag read successfully"
        // We don't invalidate the session to allow for continuous reading
      }
